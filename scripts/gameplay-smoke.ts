...
async function main() {
  ...
  try {
    ...
    writeFileSync(join(directory, 'server.properties'), [
      ...
      'level-name=fixture-world', 'difficulty=peaceful', 'gamemode=survival',
    ...
    await until(() => ['furnace', 'chicken', 'oak_planks', 'wooden_pickaxe'].every(name => bot!.inventory.items().some(i => i.name === name)), 'fixture_items');
    ...
    console.log('REAL_SERVER_SMOKE_PASSED: place/open/transfer/process-output/close/equip/break/move/pickup');
  } finally {
