...
### 6. infra/scripts/systemd/ffmpeg-stream.service
```
- -c:v libx264 -preset ultrafast -tune zerolatency -b:v 3000k ... -r 24
+ -c:v h264_nvenc -preset p4 -b:v 4500k -maxrate 5000k -bufsize 10000k

- -framerate 24
+ -framerate 30

- CPUAffinity=2
+ (削除: GPU がエンコードするので CPU ピンニング不要)
```
